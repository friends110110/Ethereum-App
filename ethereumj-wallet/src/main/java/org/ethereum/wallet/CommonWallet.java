...
public class CommonWallet implements Wallet {
     ...
     * 之前是 262144
     */
    private static Integer mN=2048;
    ...
    public static Wallet fromV3(String keyfile_json, String password) throws GeneralSecurityException {
        ...
        Map json;
        try {
            json = JsonReader.jsonToMaps(keyfile_json);
            if (json == null) {
        ...
        if ("scrypt".equalsIgnoreCase(kdf)) {
            ...
            int p = kdfparams.containsKey("p") ? ((Long) kdfparams.get("p")).intValue() : 1;
            int dkLen = kdfparams.containsKey("dklen") ? ((Long) kdfparams.get("dklen")).intValue() : 32;
            int r = kdfparams.containsKey("r") ? ((Long) kdfparams.get("r")).intValue() : 8;
            int n = kdfparams.containsKey(mN.toString()) ? ((Long) kdfparams.get(mN.toString())).intValue() : mN;
            ...
            int dkLen = kdfparams.containsKey("dklen") ? ((Long) kdfparams.get("dklen")).intValue() : 256;
            int c = kdfparams.containsKey("c") ? ((Long) kdfparams.get("c")).intValue() : mN;
        ...
        try {
            ...
            IvParameterSpec iv_spec = new javax.crypto.spec.IvParameterSpec(Hex.decode(iv));
